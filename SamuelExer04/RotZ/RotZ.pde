...
void setup() {
  ...
  for(int i =0 ; i < xAx.length;i++) {
    float rotX = xAx[i] * cos(radians(rots[x])) - yAx[i] * sin(radians(rots[x]));
    float rotY = xAx[i] * sin(radians(rots[x])) + yAx[i] * cos(radians(rots[x]));
    output.println(rotX + "\t"+ rotY +"\t" + zAx[i]);
  }
