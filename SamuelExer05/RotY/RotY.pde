...
void setup() {
  ...
  for(int i =0 ; i < xAx.length;i++) {
    float rotX = xAx[i] * cos(radians(rots[x])) - zAx[i] * sin(radians(rots[x]));
    float rotZ = xAx[i] * sin(radians(rots[x])) + zAx[i] * cos(radians(rots[x]));
    output.println(rotX + "\t"+ yAx[i] +"\t" + rotZ);
  }
